...
#Empty model
model0 <- glm(language.of.instruction~1, data = PushFactors,family = binomial(link = "logit"))

summary(model0)


#Full Model
model1 <- glm(language.of.instruction~encouragement.from.family.friends+
...
model2<- glm(language.of.instruction~encouragement.from.family.friends+
               advantages.of.studying.abroad+access.to.education+
               Competitive.University.admission.process+Perceived.advantage.of.international.degree+
               Unavailability.of.scholarship.opportunities+To.experience.a.different.culture+
               as.numeric(age)+as.numeric(Gender)+as.numeric(Home.country), 
             data = PushFactors,family = binomial(link = "logit"))

summary(model2)

#Full Model with interaction effect
model3<- glm(language.of.instruction~Unavailability.of.scholarship.opportunities+Perceived.advantage.of.international.degree+
               (encouragement.from.family.friends+
               advantages.of.studying.abroad+access.to.education+
               Competitive.University.admission.process+
               To.experience.a.different.culture)*world.region+
...
model3<-glm(language.of.instruction~advantages.of.studying.abroad+
             ...
             (access.to.education+encouragement.from.family.friends+Perceived.advantage.of.international.degree+
                To.experience.a.different.culture)*world.region, data = PushFactors,family = binomial(link = "logit"))

summary(model3)


model4<-glm(language.of.instruction~advantages.of.studying.abroad+
             Unavailability.of.scholarship.opportunities+Competitive.University.admission.process+
             (access.to.education+encouragement.from.family.friends+Perceived.advantage.of.international.degree+
                To.experience.a.different.culture)*world.region+as.numeric(age)+
             as.numeric(Gender), data = PushFactors, family = binomial(link = "logit"))
...
stargazer(model1, model2, title="Regression Results: Push Factors in Home Country", align=TRUE,
          no.space=TRUE, single.row=T,type = "text", intercept.bottom = F,dep.var.labels = "Move to Moscow", out="aaaa.doc",
          covariate.labels =c("Constant","Encouragement from family and friends","Advantages of studying abroad",
...
stargazer(model3, model4, title="Regression Results: Push Factors in Home Country", align=TRUE,column.sep.width = "-5pt",
          no.space=TRUE, single.row=T,type = "text", intercept.bottom = F,dep.var.labels = "Move to Moscow", out="bbbb.doc",
...
#anova
#employment prospect
leveneTest(as.numeric(employment_prospect) ~ world.region, data = PullFactors)
ep.aov <- aov(as.numeric(employment_prospect) ~ world.region, data = PullFactors)
summary(glht(ep.aov, linfct = mcp(world.region = "Tukey")))

#proximity
leveneTest(as.numeric(geo.cul.proximity) ~ world.region, data = PullFactors)
gcp.aov <- aov(as.numeric(geo.cul.proximity) ~ world.region, data = PullFactors)
summary(glht(gcp.aov, linfct = mcp(world.region = "Tukey")))

#cost of living
leveneTest(as.numeric(living.cost) ~ world.region, data = PullFactors)
lc.aov <- aov(as.numeric(living.cost) ~ world.region, data = PullFactors)
summary(glht(lc.aov, linfct = mcp(world.region = "Tukey")))

#scholarship
leveneTest(as.numeric(available.scholarship) ~ world.region, data = PullFactors)
as.aov <- aov(as.numeric(available.scholarship) ~ world.region, data = PullFactors)
summary(glht(as.aov, linfct = mcp(world.region = "Tukey")))

#HSE quality
leveneTest(as.numeric(HSE_quality) ~ world.region, data = PullFactors)
hq.aov <- aov(as.numeric(HSE_quality) ~ world.region, data = PullFactors)
summary(glht(hq.aov, linfct = mcp(world.region = "Tukey")))

#HSE reputation
leveneTest(as.numeric(HSE_reputation) ~ world.region, data = PullFactors)
hr.aov <- aov(as.numeric(HSE_reputation) ~ world.region, data = PullFactors)
summary(glht(hr.aov, linfct = mcp(world.region = "Tukey")))

#Choice of program
leveneTest(as.numeric(program_choice) ~ world.region, data = PullFactors)
pc.aov <- aov(as.numeric(program_choice) ~ world.region, data = PullFactors)
summary(glht(pc.aov, linfct = mcp(world.region = "Tukey")))

#Attraction to Russian culture
leveneTest(as.numeric(russian.culture) ~ world.region, data = PullFactors)
rc.aov <- aov(as.numeric(russian.culture) ~ world.region, data = PullFactors)
summary(glht(rc.aov, linfct = mcp(world.region = "Tukey")))

#recommendations from family and friends
leveneTest(as.numeric(family.friends.recommendations) ~ world.region, data = PullFactors)
ffr.aov <- aov(as.numeric(family.friends.recommendations) ~ world.region, data = PullFactors)
summary(glht(ffr.aov, linfct = mcp(world.region = "Tukey")))

#HSE rank
leveneTest(as.numeric(HSE.rank) ~ world.region, data = PullFactors)
hrk.aov <- aov(as.numeric(HSE.rank) ~ world.region, data = PullFactors)
summary(glht(hrk.aov, linfct = mcp(world.region = "Tukey")))

#Support for international students
leveneTest(as.numeric(international.students.support) ~ world.region, data = PullFactors)
iss.aov <- aov(as.numeric(international.students.support) ~ world.region, data = PullFactors)
summary(glht(iss.aov, linfct = mcp(world.region = "Tukey")))

#HSE alliances
leveneTest(as.numeric(HSE.alliances) ~ world.region, data = PullFactors)
ha.aov <- aov(as.numeric(HSE.alliances) ~ world.region, data = PullFactors)
summary(glht(ep.aov, linfct = mcp(world.region = "Tukey")))
